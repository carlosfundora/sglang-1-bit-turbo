...
sys.modules["sglang.srt.mem_cache.base_prefix_cache"].MatchPrefixParams = type('MatchPrefixParams', (), {'__init__': lambda self, key: setattr(self, 'key', key)})
sys.modules["sglang.srt.mem_cache.base_prefix_cache"].MatchResult = type('MatchResult', (), {'__init__': lambda self, value, last_node: [setattr(self, 'value', value), setattr(self, 'last_node', last_node), None][-1]})
...
TreeNode = mamba_radix_cache.TreeNode
MatchPrefixParams = sys.modules["sglang.srt.mem_cache.base_prefix_cache"].MatchPrefixParams

class DummyMambaRadixCache:
    def __init__(self):
        self.root_node = TreeNode()
        self.root_node.key = tuple()
        self.root_node.value = []
        self.root_node.children = {}

        self.key_match_fn = mamba_radix_cache._key_match_page_size1
        # Patch the method dynamically to test
        self.match_prefix = mamba_radix_cache.MambaRadixCache.match_prefix.__get__(self)
        self._split_node = mamba_radix_cache.MambaRadixCache._split_node.__get__(self)

    def populate(self):
        # build a fake tree
        node1 = TreeNode()
        node1.key = tuple(range(100))
        node1.value = list(range(100))
        self.root_node.children[0] = node1

        node2 = TreeNode()
        node2.key = tuple(range(100, 200))
        node2.value = list(range(100, 200))
        node1.children[100] = node2

def benchmark():
    cache = DummyMambaRadixCache()
    cache.populate()
    ...
    search_keys = [
        ...
        tuple(range(100)),
        tuple(range(150)),
        tuple(list(range(100)) + [999] + list(range(101, 150)))
    ] * 25000  # 100,000 iterations

    start_time = time.perf_counter()
    for key in search_keys:
        cache.match_prefix(MatchPrefixParams(key))
    ...
    import datetime
    out = {
        "candidate": "python/sglang/srt/mem_cache/mamba_radix_cache.py",
        "implementation": "after",
        "command": "python bench_mamba.py",
        "timestamp": datetime.datetime.now().isoformat(),
        "iterations": len(search_keys),
        "input_description": "100,000 match_prefix queries of varying lengths",
        "duration_ms": duration,
        "throughput": f"{len(search_keys) / (duration / 1000.0):.2f} req/s",
        "notes": "Uses pure Rust string slice iteration"
    }

    os.makedirs(".jules/verification/rusty", exist_ok=True)
    with open(".jules/verification/rusty/after-benchmark.json", "w") as f:
        json.dump(out, f, indent=2)
    print(f"Benchmark completed: {duration:.2f} ms")
