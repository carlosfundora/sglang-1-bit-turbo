...
try:
    from sglang.sglang_rust_utils import parse_message_from_completion_text_dsv32 as rust_parse_message
except ImportError:
    try:
        from sglang_rust_utils import parse_message_from_completion_text_dsv32 as rust_parse_message
    except ImportError:
        rust_parse_message = None
...
def parse_message_from_completion_text(text: str, thinking_mode: str):
    if rust_parse_message is not None:
        try:
            return rust_parse_message(text, thinking_mode)
        except ValueError as e:
            raise DS32EncodingError(str(e))
