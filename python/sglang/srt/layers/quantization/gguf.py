...
class GGUFLinearMethod(LinearMethodBase):
    ...
    def apply(
        ...
        if shard_id:
            ...
            # padded weight. If shards are in order, use output directly.
            # If out of order, slice and reorder the output columns.
            ...
            if len(set(shard_types)) == 1:
                ...
                if len(set(dim1_sizes)) == 1:
                    ...
                    # Check if padded tensor is in shard_id iteration order
                    in_order = (
                        offsets[0][0] == 0
                        and all(
                    ...
                    if not in_order:
                        # Reorder output columns: fused output is in
                        # data_container (load) order, we need shard_id order.
                        shard_size = offsets[0][1] - offsets[0][0]
                        slices = []
