...
// Ported from llama.cpp-1-bit-turbo CUDA kernels
...
#pragma unroll
    for (int j = 0; j < 8; ++j) {
      ...
      // Convert each of 4 bits to signed byte: bit=1 → +1, bit=0 → -1
...
#pragma unroll
  ...
  // Q1_0 is symmetric (no zero-point offset), so result = d1 * d8 * sumi
  return d1 * ds8f.x * sumi;
}

// MMVQ wrapper for Q1_0 (32 elements per block)
// Signature matches vec_dot_q_cuda_t: vbq is already offset by ibx in mul_mat_vec_q
static __device__ __forceinline__ float vec_dot_q1_0_q8_1(
  ...
  // Pack all 4 bytes (32 bits) of the Q1_0 block into one int32
  v[0] = bq1_0->qs[0] | (bq1_0->qs[1] << 8) | (bq1_0->qs[2] << 16) | (bq1_0->qs[3] << 24);

  // Load 8 int32s from Q8_1 (each holding 4 int8 activation values, covering 32 elements)
#pragma unroll
  for (int j = 0; j < 8; ++j) {
...
static __device__ __forceinline__ float vec_dot_q1_0_g128_q8_1(
  ...
  // Load 32 bits (4 bytes) for this chunk
  ...
  // Unpack 32 bits into ±1 signed bytes and dp4a against Q8_1
  int vi_bytes[8];
#pragma unroll
  for (int j = 0; j < 8; ++j) {
    const int shift = j * 4;
