...
## 2026-05-28 - [Vectorized CPU-GPU synchronizations in compute_position_torch]
**Learning:** `compute_position_torch` was identified as another place transferring full arrays back to the CPU via `.cpu()` alongside using a list comprehension (`for x,y in zip()`) to concatenate multiple individually allocated `torch.arange` operations on the CPU and moving them back to the GPU. This is an immense performance killer under heavy load.
**Action:** Replace Python iterative tensor allocation with vectorized `torch.repeat_interleave` and `torch.cumsum` computations to stay entirely on the device. Only pull masks back to the CPU using `.any().item()` when handling boundary edge cases or error logging.
