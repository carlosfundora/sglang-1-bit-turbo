...
# I noticed the previous fixes just duplicated the imports block because the string replace matched multiple things or I wrote it multiple times over iterations without checking git state
# Let's cleanly patch it by inserting just once at the VERY TOP
lines = content.split('\n')
for i, line in enumerate(lines):
    if line.startswith("import heapq"):
        # Found the first standard import
        lines.insert(i, "try:")
        lines.insert(i+1, "    from sglang.sglang_rust_utils import mamba_match_prefix")
        lines.insert(i+2, "    RUST_UTILS_AVAILABLE = True")
        lines.insert(i+3, "except ImportError:")
        lines.insert(i+4, "    RUST_UTILS_AVAILABLE = False")
        lines.insert(i+5, "")
        break
